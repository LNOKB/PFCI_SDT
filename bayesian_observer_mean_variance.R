...
for (j in 1:length(subs)){
  estvec <- unlist(unname(results[j, 1:16])) #1:12
  estimates[j, ] <- estvec
  estimates[j, 16] <- -estimates[j, 16] 
  
  predvec <- unlist(unname(results[j, 17:58])) #13:54
  pred_data <- matrix(predvec, nrow = 21, ncol = 2, byrow = TRUE)
  predicted_array[, , j] <- pred_data
  
  datavec <- unlist(unname(results[j, 59:100])) #55:96
...
for (k in 1:length(subs)){
  #47
  #Bar plot
  if (any(is.na(estimates[k, ]))) {
    message(paste("Skipping subject", subs[k], "due to NA in estimates"))
    next
  }
  ...
  bar_graph_sub <- ggplot(data_bar_sub, aes(x = Imagetype, y = Proportion)) +
    ...
    geom_text(data = ann_text, label = paste("Log-likelihood =",  round(estimates[k, 16], 1)),
              size = 7,
              color = "red") #12
  
  ggsave(file = file.path("SubPlotBar_both", paste0("bar_graph_bayes_both_", subs[k], ".png")),
  ...
  plot_sdt_distributions_ideal_sub <- function(means, sds, attention_levels, image_types, colors) {
    ...
    ggsave(file = file.path("SubPlotDistribution_both", paste0("Distribution_bayes_both_", subs[k], ".png")),
  ...
  sd_117ms <- c(rep(estimates[k, 14], 7))
  sd_150ms <- c(rep(estimates[k, 15], 7))
