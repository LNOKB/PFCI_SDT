...
est_names <- c("mu83ms_9deg", "mu83ms_13deg", "mu83ms_17deg", "mu83ms_21deg", "mu83ms_25deg",
               ...
               "prior_chimera_levels","prior_fullcolor", "sigma117ms", "sigma150ms", "logL")
par_names <- c("initial_prior","parscales_mu","parscales_sigma_lambda","parscales_prior")
all_names <- c(est_names, par_names)
...
initial_prior_pool <- seq(0.125, 0.625, length.out = 11)
parscales_mu_pool <- seq(30, 100, length.out = 6)
parscales_sigma_lambda_pool <- seq(5, 30, length.out = 6)
parscales_prior_pool <- seq(0.5, 10, length.out = 20)
...
posterior_probs <- function(x, mu83ms_chimeras, mu83ms_color, lambdas, prior_fullcolor, sigmas) {
  criteria <- numeric(3)  
  ...
  for (cond2 in 1:3) {
    ...
    numerator_chimera_levels <- sapply(mu83ms_chimeras, function(mu) {
      dnorm(x, mean = mu * lambdas[cond2], sd = sigmas[cond2]) * prior_chimera_levels
    })
    numerator_chimera <- rowSums(numerator_chimera_levels)
    numerator_gray <- dnorm(x, mean = mu83ms_gray * lambdas[cond2], sd = sigmas[cond2]) * prior_gray
    numerator_full <- dnorm(x, mean = mu83ms_color * lambdas[cond2], sd = sigmas[cond2]) * prior_fullcolor
...
fit_PFCI_mle <- function(data, add_constant = TRUE) {
  ...
  initial_sigma117ms <- 1
  initial_sigma150ms <- 1

  lower_bounds <- c(0,   0,   0,   0,   0,   0,   0.5, 0.5, 0.125, 0.5, 0.5)
  upper_bounds <- c(3.5, 3.5, 3.5, 3.5, 3.5, 3.5, 2.0, 2.0, 0.675, 2.0, 2.0)
...
PFCI_logL <- function(x) {
  ...
  sigmas <- c(sigma83ms, sigma117ms, sigma150ms)
  criteria <- posterior_probs(x_seq, mu83ms_chimeras, mu83ms_color, lambdas, prior_fullcolor, sigmas)
  
  #Prediction of full-color / other response 
  mean_gray_mat <- mu83ms_gray * lambdas
  for (cond in 1:3) {
    predicted_data[cond, 1] <- pnorm(criteria[cond], mean = mean_gray_mat[cond], sd = sigmas[cond]) # pred_gray_others_rate
  ...
  sigma_chimera_mat <- rep(sigmas, each = length(mu83ms_chimeras))
  criteria_chimera <- rep(criteria, each = 5)
  for (cond in 4:18) {
    predicted_data[cond, 1] <- pnorm(criteria_chimera[cond-3], mean = mean_chimera_mat[cond-3], 
                                     sd = sigma_chimera_mat[cond-3]) # pred_chimera_others_rate
    predicted_data[cond, 2] <- 1 - predicted_data[cond, 1] # pred_chimera_color_rate
  }
  
  mean_color_mat <- mu83ms_color * lambdas
  for (cond in 19:21) {
    predicted_data[cond, 1] <- pnorm(criteria[cond-18], mean = mean_color_mat[cond-18], 
                                     sd = sigmas[cond-18]) # pred_color_others_rate
...
estimates <- data.frame(matrix(NA, nrow = 44, ncol = length(all_names)))
colnames(estimates) <- all_names
...
results <- foreach(i = subs, .combine = 'rbind') %dopar% {
  ...
  df <- data.frame(matrix(NA, nrow=1, ncol=length(c(est_names, names_pred, names_rate, par_names))))
  
  for (i in 1:n_trials) {
    # ランダムな組み合わせ
    initial_prior <- sample(initial_prior_pool, size = 1, replace = TRUE)
    parscales_mu <- sample(parscales_mu_pool, size = 1, replace = TRUE)
    parscales_sigma_lambda <- sample(parscales_sigma_lambda_pool, size = 1, replace = TRUE)
    parscales_prior <- sample(parscales_prior_pool, size = 1, replace = TRUE)
    parscales <- c(parscales_mu, parscales_mu, parscales_mu, parscales_mu, parscales_mu, parscales_mu,
                   parscales_sigma_lambda, parscales_sigma_lambda,
                   parscales_prior, parscales_sigma_lambda, parscales_sigma_lambda)
    ...
    df <- data.frame(t(c(est_vec, pred_vec, rate_vec, initial_prior, 
                         parscales_mu, parscales_sigma_lambda, parscales_prior)))
    
    # cat("initial_prior:", paste(initial_prior, collapse = ", "), "\n")
    # cat("parscales:", paste(parscales, collapse = ", "), "\n\n")
    
    # NAがない場合にのみcatで報告
    # if (!any(is.na(estimates[(i - 3), ]))) {
    #   cat("initial_prior:", paste(initial_prior, collapse = ", "), "\n")
    #   cat("parscales:", paste(parscales, collapse = ", "), "\n\n")
    #   break
    # }
  ...
  colnames(df) <- c(est_names, names_pred, names_rate, par_names) 
  # df$sub <- i
  df
}
...
# for (i in subs){
#   estvec <- unlist(unname(results[(i - 3), 1:16]))
#   estimates[(i - 3), 1:16] <- estvec
#   estimates[(i - 3), 16] <- -estimates[(i - 3), 16]
# 
#   estimates[(i - 3), 17:20] <- unlist(unname(results[(i - 3), 101:104]))
# 
#   predvec <- unlist(unname(results[(i - 3), 17:58]))
#   pred_data <- matrix(predvec, nrow = 21, ncol = 2, byrow = TRUE)
#   predicted_array[, , (i - 3)] <- pred_data
# 
#   datavec <- unlist(unname(results[(i - 3), 59:100]))
#   data_rate <- matrix(datavec, nrow = 21, ncol = 2, byrow = TRUE)
#   data_rate_array[, , (i - 3)] <- data_rate
# }
# 
...
#   #Bar plot
#   if (any(is.na(estimates[(i - 3), ]))) {
#     message(paste("Skipping subject", i, "due to NA in estimates"))
#     next
#   }
#   
...
#     ) +
#     geom_text(data = ann_text, label = paste("Log-likelihood =",  round(estimates[(i - 3), 16], 1)),
#               size = 7,
#               color = "red") #12
#   
#   ggsave(file = file.path("SubPlotBar_bayes_both", paste0("bar_graph_bayes_both_", i, ".png")),
...
#       )
#     
#     ggsave(file = file.path("SubPlotDistribution_bayes_both", paste0("Distribution_bayes_both_", subs[k], ".png")),
...
#   sd_117ms <- c(rep(estimates[(i - 3), 14], 7))
#   sd_150ms <- c(rep(estimates[(i - 3), 15], 7))
...
#                      "5.θBayes150ms", "6.sigma117ms", "7.sigma150ms"), each = 44),
#   Value = c(estimates[, 7], estimates[, 8], estimates[, 9], estimates[, 10], 
#             estimates[, 11],estimates[, 14],estimates[, 15])
# )
...
# ggsave(file = "parameters_graph_bayes_both.png", plot = parameters_graph, dpi = 150, width = 12, height = 6)
# 
# 
# # violin plot
# data_parameter_plot <- data.frame(
...
#   )
# plot(parameters_graph)
# ggsave(file = "parameters_graph_bayes_both_π.png", plot = parameters_graph, dpi = 150, width = 3, height = 3)
# 
# 
# 
...
# 
#   plot(Distribution)
#   ggsave(file = "Distribution_bayes_both.png", plot = Distribution, dpi = 150, width = 8, height = 6)
# 
# 
# }
# 
...
# sd_117ms <- c(rep(mean(estimates[, 14]), 7))
# sd_150ms <- c(rep(mean(estimates[, 15]), 7))
# sds <- rbind(sd_83ms, sd_117ms, sd_150ms)
# 
# attention_levels <- factor(c("83 ms", "117 ms", "150 ms"), levels = c("83 ms", "117 ms", "150 ms"))
# image_types <- factor(
...
#     legend.title = element_text(size = 18)  #
#   ) +
#   geom_text(data = ann_text, label = paste("Summed log-likelihood =",  round(sum(estimates[, 16]), 1)),
#             size = 7,
#             color = "red")
# 
# # scale_y_continuous(
...
# 
# plot(bar_graph)
# ggsave(file = "bar_graph_bayes_both.png", plot = bar_graph, dpi = 150, width = 14, height = 8)
# #End of Mean plot
# 
# 
