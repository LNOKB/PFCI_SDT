...
fit_PFCI_mle <- function(data, add_constant = TRUE) {
  ...
  guess <- c(
    ...
    1, 1, 2         # lambda117ms, lambda150ms,  theta 
  )
...
PFCI_logL <- function(x, inputs) {
  ...
  for (cond in 1:3) {
    predicted_data[cond, 1] <- pnorm(theta, mean = mean_one_mat[cond], sd = sigma) # pred_gray_others_rate
  ...
  for (cond in 4:18) {
    predicted_data[cond, 1] <- pnorm(theta, mean = mean_one_mat[cond - 3], sd = sigma) # pred_chimera_others_rate
  ...
  for (cond in 19:21) {
    predicted_data[cond, 1] <- pnorm(theta, mean = mean_two_mat[cond - 18], sd = sigma) # pred_color_others_rate
...
data_parameter_plot <- data.frame(
  Parameters = rep(c("λ117ms", "λ150ms"), each = 44),
...
plot_sdt_distributions <- function(means, sds, attention_levels, image_types, colors) {
  ...
  plot(Distribution)
  ggsave(file = "Distribution2.png", plot = Distribution, dpi = 100, width = 16, height = 12)
...
plot(bar_graph)
ggsave(file = "bar_graph2.png", plot = bar_graph, dpi = 100, width = 21, height = 12)
