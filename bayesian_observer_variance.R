...
est_names <- c("mu_9deg", "mu_13deg", "mu_17deg", "mu_21deg", "mu_25deg",
               "mu_color", "sigma117ms", "sigma150ms", "theta83ms", "theta117ms", "theta150ms", 
               "prior_chimera_levels","prior_fullcolor", "logL")


posterior_probs <- function(x, mu_chimeras, mu_color, sigmas, prior_fullcolor) {
  criteria <- numeric(3)  
  ...
  for (cond2 in 1:3) {
    ...
    numerator_chimera_levels <- sapply(mu_chimeras, function(mu) {
      dnorm(x, mean = mu, sd = sigmas[cond2]) * prior_chimera_levels
    })
    numerator_chimera <- rowSums(numerator_chimera_levels)
    numerator_gray <- dnorm(x, mean = mu_gray , sd = sigmas[cond2]) * prior_gray
    numerator_full <- dnorm(x, mean = mu_color , sd = sigmas[cond2]) * prior_fullcolor
...
fit_PFCI_mle <- function(data, add_constant = TRUE) {
  ...
  if (i %in% c(6, 10, 11, 18:22, 24, 25, 27, 32, 34, 36, 39, 45)) {
    initial_prior <- 2.5/12
    min_prior <- 1.5/12
    max_prior <- 7.5/12
    parscales <- c(100, 100, 100, 100, 100, 100, 10, 10, 0.5)
  } else if (i %in% c(4, 9, 12, 17, 29, 37, 40)) {
    initial_prior <- 5/12
    min_prior <- 2.5/12
    max_prior <- 7.5/12
    parscales <- c(100, 100, 100, 100, 100, 100, 10, 10, 30)
  } else if (i %in% c(15, 16, 26, 31, 33, 35, 43, 46, 47)) {
    initial_prior <- 2.5/12
    min_prior <- 1.5/12
    max_prior <- 6/12
    parscales <- c(100, 100, 100, 100, 100, 100, 10, 10, 0.5)
  } else if (i %in% c(28, 41)) {
    initial_prior <- 3/12
    min_prior <- 2/12
    max_prior <- 5/12
    parscales <-  c(30, 30, 30, 30, 30, 30, 20, 20, 5)
  } else if (i %in% c(7, 8, 30, 38, 44)) {
    initial_prior <- 2.3/12
    min_prior <- 2/12
    max_prior <- 5/12
    parscales <- c(30, 30, 30, 30, 30, 30, 10, 10, 0.5)
  } else if (i %in% c(5, 13, 23)) {
...
PFCI_logL <- function(x) {
  ...
  sigmas <- c(1, sigma117ms, sigma150ms) 
  criteria <- posterior_probs(x_seq, mu_chimeras, mu_color, sigmas, prior_fullcolor)
  
  #Prediction of full-color / other response 
  for (cond in 1:3) {
    predicted_data[cond, 1] <- pnorm(criteria[cond], mean = mu_gray, sd = sigmas[cond]) # pred_gray_others_rate
    predicted_data[cond, 2] <- 1 - predicted_data[cond, 1] # pred_gray_color_rate
  }
  
  mean_chimera_mat <- c(rep(mu_chimeras,3))
  ...
  for (cond in 4:18) {
    predicted_data[cond, 1] <- pnorm(criteria_chimera[cond-3], mean = mean_chimera_mat[cond-3], sd = sigma_mat[cond-3]) # pred_chimera_others_rate
    predicted_data[cond, 2] <- 1 - predicted_data[cond, 1] # pred_chimera_color_rate
  }
  
  for (cond in 19:21) {
    predicted_data[cond, 1] <- pnorm(criteria[cond-18], mean = mu_color, sd = sigmas[cond-18]) # pred_color_others_rate
...
for (k in 1:length(subs)){
  ...
  ggsave(file = file.path("SubPlotBar1", paste0("bar_graph_bayes_", subs[k], ".png")),
  ...
  plot_sdt_distributions_ideal_sub <- function(means, sds, attention_levels, image_types, colors) {
    ...
    ggsave(file = file.path("SubPlotDistribution1", paste0("Distribution_bayes_", subs[k], ".png")),
           plot = Distribution_ideal_sub, dpi = 150, width = 8, height = 6)
  }

  means_83ms <- c(mu_gray, estimates[k, 1], estimates[k, 2],
  ...
  sd_117ms <- c(rep(estimates[k, 7], 7))
  sd_150ms <- c(rep(estimates[k, 8], 7))
...
data_parameter_plot <- data.frame(
  Parameters = rep(c("1.σ117ms", "2.σ150ms", "3.θBayes83ms", "4.θBayes117ms", "5.θBayes150ms"), each = 44),
...
ggsave(file = "parameters_graph_bayes_variance.png", plot = parameters_graph, dpi = 150, width = 12, height = 6)
...
plot_sdt_distributions <- function(means, sds, attention_levels, image_types, colors) {
  ...
  plot(Distribution)
  ggsave(file = "Distribution_bayes_variance.png", plot = Distribution, dpi = 150, width = 8, height = 6)
...
plot(bar_graph)
ggsave(file = "bar_graph_bayes_variance.png", plot = bar_graph, dpi = 150, width = 14, height = 8)
#End of Mean plot