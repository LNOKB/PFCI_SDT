...
fit_PFCI_mle <- function(data, add_constant = TRUE) {
  ...
  grid <- seq(1/12.5, 1/11.5, length = 20)  
  fitresults <- as.data.frame(matrix(0, nrow = length(grid), ncol = length(col_names)))
  colnames(fitresults) <- col_names
  
  for (iii in seq_along(grid)) {
    prior_chimera_levels <- grid[iii]
    guess <- c(
      initial_mu(data, 4),
      initial_mu(data, 5),
      initial_mu(data, 6),
      ...
      initial_lambda117ms,
      initial_lambda150ms,
      prior_chimera_levels  #グリッドで動かす
    )
    
    lower_bounds <- c(0, 0, 0, 0, 0, 0, 0.5, 0.5, prior_chimera_levels) 
    upper_bounds <- c(3.5, 3.5, 3.5, 3.5, 3.5, 3.5, 2.0, 2.0, prior_chimera_levels) 
    
    control_params <- list(
      "maxit" = 10000,
      "parscale" = c(1, 1, 1, 1, 1, 1, lambda_parscale, lambda_parscale, 1) #, 1
    )
    ...
    predicted_data_list[[iii]] <- fitresult$predicted_data
    
    criteria <- fitresult$criteria
    fitresults[iii, col_names] <- c(
      fit$par[1:8],
      criteria[1:3],
      prior_chimera_levels,
      1 - (prior_gray + prior_chimera_levels * 5),
      fit$value
    )
  ...
  # choose the best 
  best_idx <- which.min(fitresults$logL)
  best_result <- fitresults[best_idx, ]
  best_predicted_data <- predicted_data_list[[best_idx]]
  est <- as.data.frame(best_result[col_names])

  return(list(est = est, predicted_data = best_predicted_data))
}
